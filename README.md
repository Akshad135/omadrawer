...
- **No Manager Icon:** The bar shows only your groups. The manager opens via right-click on a group header or `omarchy-shell akshad135.omadrawer toggle`.
...
- **IPC & CLI Integration:** Open, close, toggle, or expand a specific group via `omarchy-shell akshad135.omadrawer <action>`.
...
- **CLI / Keybinding:**
  - `omarchy-shell akshad135.omadrawer toggle` — open or close the manager.
  - `omarchy-shell akshad135.omadrawer toggleGroup <groupId>` — expand or collapse a specific drawer.
  - `omarchy-shell akshad135.omadrawer reload` — reload groups from disk.
...
All preferences and group definitions persist safely in `~/.local/state/omarchy/plugins/akshad135.omadrawer/groups.json`.
...
```bash
omarchy plugin disable akshad135.omadrawer
omarchy plugin remove akshad135.omadrawer
```
...
- **State Storage:** Settings and group configurations persist safely in `~/.local/state/omarchy/plugins/akshad135.omadrawer/groups.json`.
